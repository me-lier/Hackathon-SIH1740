...
def plot_gnss_data_on_map(data_file, output_map_file, circle_center=None, circle_radius=50):
    ...
    if 'Latitude' in df.columns and 'Longitude' in df.columns and 'Timestamp' in df.columns:
        ...
        # Parse the 'matched_road' WKT string into Latitude and Longitude
        df['matched_road'] = df['matched_road'].apply(wkt.loads)
        df['matched_lat'] = df['matched_road'].apply(lambda x: x.y)
        df['matched_lon'] = df['matched_road'].apply(lambda x: x.x)
        ...
        for _, row in df.iterrows():
            ...
            # Add matched road points to the map
            folium.Marker(
                location=[row['matched_lat'], row['matched_lon']],
                popup=f"Matched Road Point: {row['Timestamp']}",
...
if __name__ == "__main__":
    # File paths
    data_file = '../Location_data/Rahil/Data4/matched_gps_to_road.csv'
    output_map_file = '../output/Rahil/Data4/gnss_map_matched.html'
    
    # Coordinates for the circle's center (latitude, longitude)
    circle_center = (22.292576, 73.271293)  # Example coordinates
    
    # Plot GNSS data on the map with a circle of 1200 meters radius
